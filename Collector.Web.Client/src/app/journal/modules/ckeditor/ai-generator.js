/**
 * AI Generator plugin for CKEditor
 * This plugin adds an AI Content Generator button to the toolbar
 */

// Define the plugin as a factory function to avoid duplicate module registration
export default function defineAIGeneratorPlugin(Plugin, ButtonView) {
    // UI plugin class
    class AIGeneratorUI extends Plugin {
        init() {
            const editor = this.editor;
            const t = editor.t;

            // Register the AI Generator button
            editor.ui.componentFactory.add('aiGenerator', locale => {
                const button = new ButtonView(locale);

                button.set({
                    label: t('AI Content Generator'),
                    icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><rect fill="none" width="24" height="24"/><g fill="currentColor"><path d="
                    M 8.9 13.95
...
" /></g></svg>`,
                ...
                // Execute the command on button click
                button.on('execute', () => {
                    // Dispatch a custom event that will be handled in the main CKEditor component
                    const customEvent = new CustomEvent('aiGeneratorRequest', {
                        detail: { editor }
                    });
    ...
    // Main plugin class
    class AIGenerator extends Plugin {
        static get requires() {
