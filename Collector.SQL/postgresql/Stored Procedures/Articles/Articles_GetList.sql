CREATE OR REPLACE FUNCTION public."Articles_GetList"
(
...
RETURNS TABLE(
    ...
    "breadcrumb" VARCHAR(500), "hierarchy" VARCHAR(50), "subjectTitle" VARCHAR(50)
)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    SELECT * FROM (
        SELECT ROW_NUMBER() OVER(ORDER BY
            CASE WHEN p_orderBy = 0 THEN a."title" END ASC,
            CASE WHEN p_orderBy = 1 THEN a."title" END DESC,
            CASE WHEN p_orderBy = 2 THEN a."url" END ASC,
            CASE WHEN p_orderBy = 3 THEN a."url" END DESC,
            CASE WHEN p_orderBy = 4 THEN a."score" END ASC,
            CASE WHEN p_orderBy = 5 THEN a."score" END DESC,
            CASE WHEN p_orderBy = 6 THEN a."datecreated" END DESC,
            CASE WHEN p_orderBy = 7 THEN a."datecreated" END ASC,
            CASE WHEN p_orderBy = 8 THEN a."visited" END DESC
        ) AS rownum, a.*,
        s."breadcrumb", s."hierarchy", s."title" AS subjectTitle
        FROM public."Articles" a
        LEFT JOIN public."Subjects" s ON s."subjectId" = a."subjectId"
        WHERE (
